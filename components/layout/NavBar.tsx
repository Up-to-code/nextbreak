...
function NavBar() {
  ...
  useEffect(() => {
    ...
    // Only animate when items are added (count increases)
  ...
  const cartAnimationClasses = isCartAnimating 
    ? "animate-bounce" 
    : "";
  ...
  return (
    <>
      {/* Add CSS for custom shake animation */}
      <style jsx>{`
        @keyframes shake {
              ...
              <div className="relative">
                <button
                  ...
                  className={`relative p-3 border-4 border-black shadow-[4px_4px_0_0_#000] hover:shadow-[6px_6px_0_0_#000] hover:bg-gray-200 transition-all focus:outline-none focus:ring-2 focus:ring-yellow-500 disabled:opacity-50 disabled:cursor-not-allowed ${
                    isCartAnimating ? 'cart-shake' : cartAnimationClasses
                  }`}
                    ...
                    <>
                      ...
                      {totalItems() > 0 && (
                        <span className={`absolute -top-2 -right-2 bg-red-600 text-white text-xs font-extrabold w-6 h-6 flex items-center justify-center border-2 border-black ${
                          isCartAnimating ? 'animate-pulse scale-110' : 'animate-pulse'
